...
class DataAnalysis(ExploreData):
    ...
    def do(
        ...
        number_of_examples_in_each_class = [class_counts[i] for i in range(number_of_classes)]
        ...
        for cls in class_names:
            ...
            for i, sample_idx in enumerate(class_samples):
                img = self.datamodule.train_dataloader().dataset[sample_idx]["input"].numpy()
                axes[i].imshow(img, cmap='gray')
                axes[i].set_title(f'Class: {cls}')
                axes[i].axis('off')

            MatplotLibSaver().save(fig, self.get_step_id(), self.name, self.get_class_image_path(cls))
